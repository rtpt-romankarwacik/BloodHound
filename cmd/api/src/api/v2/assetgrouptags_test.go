	...
	apitest.
		...
		Run([]apitest.Case{
			...
			},
			{
				...
				Setup: func() {
					tag1 := model.AssetGroupTag{ID: 1, Name: "testlabel", Type: model.AssetGroupTagTypeLabel}
					tag2 := model.AssetGroupTag{ID: 2, Name: "testtier", Type: model.AssetGroupTagTypeTier}
					mockDB.EXPECT().
						GetAssetGroupTags(gomock.Any(), gomock.Any()).
						Return(model.AssetGroupTags{
							tag1,
		...
		Run([]apitest.Case{
			...
			},
			{
				...
				Setup: func() {
					...
					mockDB.EXPECT().GetAssetGroupTag(gomock.Any(), 1).
						Return(model.AssetGroupTag{}, nil)
					mockGraphDb.EXPECT().FetchNodeByGraphId(gomock.Any(), testNode3.ID).
						Return(testNode, nil)
				},
				Test: func(output apitest.Output) {
